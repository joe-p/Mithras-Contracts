...
class APP(py.ARC4Contract, avm_version=11):
    ...
    def deposit(
        ...
        """
        py.ensure_budget(
            DEPOSIT_OPCODE_BUDGET_OPUP, fee_source=py.OpUpFeeSource.GroupCredit
        )
        ...
        assert pay_txn.amount >= DEPOSIT_MINIMUM_AMOUNT, (
    ...
    def withdraw(
        ...
        """
        py.ensure_budget(
            WITHDRAWAL_OPCODE_BUDGET_OPUP, fee_source=py.OpUpFeeSource.GroupCredit
        )
        ...
        assert Txn.sender == py.TemplateVar[py.Account](
        ...
        itxn.Payment(receiver=recipient, amount=withdrawal, fee=0).submit()

        # Transfer any extra fee to fee_recipient (e.g., the TSS)
        if fee > NULLIFIER_MBR:
            itxn.Payment(
                receiver=fee_recipient, amount=fee - NULLIFIER_MBR, fee=0
            ).submit()
